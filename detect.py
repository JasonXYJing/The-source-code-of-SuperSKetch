...
class Experiment:
    # network anomaly detection based on supersketch

    def __init__(self, file_path):
        self.file_path = file_path
        self.ssketch = None
        self.count = 0   # the number of detection
        self.throughput = 0
        self.spreader_detect = []  # detected super spreaders
        self.receiver_detect = []  # detected super receivers
        self.changer_detect = []   # detected super changers
        self.src_list = []  # list of source
        self.des_list = []  # list of destination
        self.spreader_real = None  # real super spreaders
        self.receiver_real = None  # real super receivers
        self.changer_real =  None  # real super changers
    ...
    def step1(self):
        # process subtraces in one traffic trace
        ...
        for file in files:
            ...
            # Anomaly attribute
            sip_list_abnormal = list(set(sip_list_spreader) | set(sip_list_changer))  # spreader和changer的并集
            ...
            src_des = table.groupby('Src IP')['Dst IP'].nunique()  # 分组去重获得地址及其基数
    ...
    def step2(self):
        ...
        '''
        # PEC: Precision
        pec_spreader = set(self.spreader_detect) & set(self.spreader_real)  
        pec_changer = set(self.changer_detect) & set(self.changer_real)
        pec_receiver = set(self.receiver_detect) & set(self.receiver_real)
        if len(self.spreader_detect) == 0:
            PEC_Spreader = 0
        else:
            PEC_Spreader = float(len(pec_spreader)) / len(self.spreader_detect)
        if len(self.changer_detect) == 0:
            PEC_Changer = 0
        else:
            PEC_Changer = float(len(pec_changer)) / len(self.changer_detect)
        if len(self.receiver_detect) == 0:
            PEC_Receiver = 0
        else:
            PEC_Receiver = float(len(pec_receiver)) / len(self.receiver_detect)
        '''
        ...
        # print('PEC_Spreader : {:%}'.format(PEC_Spreader))
        # print('PEC_Changer : {:%}'.format(PEC_Changer))
        # print('PEC_Receiver : {:%}'.format(PEC_Receiver))
